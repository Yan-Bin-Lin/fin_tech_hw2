...
let BuyDepositButton = $('#BuyDepositButton');
let BuyDepositCount = $('#BuyDepositCount');
let BuyDepositAmount = $('#BuyDepositAmount');


// 當按下定存執行按鍵時
BuyDepositButton.on('click', async function () {
console.log("first");
console.log(BuyDepositAmount.val());
    console.log(BuyDepositCount.val());
    ...
    waitTransactionStatus();
    // 定存
    bank.methods.BuyDeposit(parseInt(BuyDepositCount.val(), 10) ).send({
        from: nowAccount,
        gas: 3400000,
        value: web3.utils.toWei(BuyDepositAmount.val(), 'ether')
    })
        .on('receipt', function (receipt) {
        	let EventLog = receipt.events.BuyDepositEvent.returnValues;
            log(EventLog, '定存成功')
            $('#ShowBDAmount').text('金額 : ' + EventLog.value)
            $('#ShowBDCount').text('期數 : ' + EventLog.count)
...
InteruptCountButton.on('click', async function () {
    ...
    bank.methods.OutDeposit(parseInt(InteruptCount, 10)).send({
        from: nowAccount,
        gas: 3400000,
    })
        .on('receipt', function (receipt) {
            let EventLog = receipt.events.OutDepositEvent.returnValues;
            log(EventLog, '提領定存成功，金額已存入銀行帳戶')
            let count =  parseFloat($('#ShowBDCount').text().replace(/[^0-9]/g, ''), 10)

			$('#ShowOD').text(InteruptCount + ' | ' + (parseFloat(InteruptCount, 10) / count) * 0.1 )

            $('#ShowBDAmount').text('金額 : ')
            $('#ShowBDCount').text('期數 : ')
...
let EndContrait = $('#EndContrait')

EndContrait.on('click', async function () {

    let count = parseInt($('#ShowBDCount').text().replace(/[^0-9]/g, ''), 10)
